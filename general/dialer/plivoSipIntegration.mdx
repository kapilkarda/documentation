...
`178.212.35.170:5060`). **Copy it — you need it for Part C.**
...
2. Set the **Primary URI** to your QCall **SIP URI** from Part B:
   ...
   - Append **`;transport=tcp`** (recommended) — e.g. `178.212.35.170:5060;transport=tcp`.
...
- **Inbound:** Caller → your DID → Plivo → `178.212.35.170:5060` (QCall) → AI agent.
...
| Inbound call never reaches the agent                | Part C not done, or Primary URI wrong. It must be the `*178.212.35.170:5060` host **without** `sip:` prefix and **with** `;transport=tcp`. |
